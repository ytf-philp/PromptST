...
class PrefixEncoder(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.prefix_projection:
            ...
            self.embedding = torch.nn.Embedding(config.pre_seq_len, config.encoder.hidden_size)
            self.trans = torch.nn.Sequential(
                torch.nn.Linear(config.encoder.hidden_size, config.prefix_hidden_size),
                torch.nn.Tanh(),
                torch.nn.Linear(config.prefix_hidden_size, config.encoder.num_hidden_layers * 2 * config.encoder.hidden_size)
            )
        else:
            self.embedding = torch.nn.Embedding(config.pre_seq_len, config.encoder.num_hidden_layers * 2 * config.encoder.hidden_size)
...
class SpeechEncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        self.n_layer = config.encoder.num_hidden_layers
        self.n_head = config.encoder.num_attention_heads
        self.n_embd = config.encoder.hidden_size // config.encoder.num_attention_heads

        self.dropout = torch.nn.Dropout(config.hidden_dropout_prob)
        self.prefix_encoder = PrefixEncoder(config)
        self.prefix_tokens = torch.arange(self.pre_seq_len).long()

        self.layer_choice=config.layer_choice
        ...
        for _, param in self.encoder.named_parameters():
            prefix_param += param.numel()
        all_param = 0
        for name, param in self.named_parameters():
            all_param += param.numel()
        print('prompt param is {}'.format(prefix_param)) # 9860105
        print('total param is {}'.format(all_param)) # 9860105
        print("tune prompt")
    ...
    def get_prompt(self, batch_size):
        prefix_tokens = self.prefix_tokens.unsqueeze(0).expand(batch_size, -1).to(self.encoder.device)
        past_key_value = self.prefix_encoder(prefix_tokens)
        bsz, seqlen, _ = past_key_value.shape
    ...
    def forward(
        ...
        **kwargs,
        ...
        labell=0
        if(self.layer_choice is not None):
        ...
        if attention_mask is not None:
            encoder_attention_mask = self.encoder._get_feature_vector_attention_mask(
                encoder_hidden_states.shape[1],past_key_values_length, attention_mask[:,past_key_values_length:]
            )
        else:
        ...
        encoder_attention_mask=encoder_attention_mask[:,past_key_values_length:]
