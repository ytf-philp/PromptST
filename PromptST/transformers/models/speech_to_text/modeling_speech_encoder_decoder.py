    ...
    def __init__(self, config):
        ...
        if self.prefix_projection:
            ...
            self.trans = torch.nn.Sequential(
                ...
                torch.nn.Tanh(),
                torch.nn.Linear(config.prefix_hidden_size, config.num_hidden_layers * 2 * config.encoder.hidden_size)
            )
        else:
...
class SpeechEncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        for _, param in self.encoder.named_parameters():
            wav2vec_param += param.numel()

        all_param = 0
        for name, param in self.named_parameters():
            all_param += param.numel()
        print('total param is {}'.format(all_param)) # 9860105
        total_param = all_param - wav2vec_param
