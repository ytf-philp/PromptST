...
def _compute_mask_indices(
    ...
    for input_length in input_lengths:
        ...
        if(input_length == None or input_length - (mask_length - 1)<=0):
            mask_length=3
...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + attention_mask[:,:,past_key_values_length:,:]
...
#增加past_key_value选项
    def forward(
        ...
        if attention_mask is not None:
            ...
            hidden_states[~attention_mask[:,past_key_values_length:]] = 0
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    if layer_choice ==None:
                        layer_outputs = layer(
                        hidden_states, attention_mask=attention_mask, output_attentions=output_attentions,past_key_value=past_key_value[layer_num],
                    )
                    else:
                        if layer_num < layer_choice:
                            layer_outputs = layer(
                            hidden_states, attention_mask=attention_mask[:,:,past_key_values_length:,past_key_values_length:], output_attentions=output_attentions,past_key_value=None)
                        else:
                            layer_outputs = layer(
                            hidden_states, attention_mask=attention_mask, output_attentions=output_attentions,past_key_value=past_key_value[layer_num],
                        )
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def _get_feature_vector_attention_mask(
        self, feature_vector_length: int, past_key_value_length:int,attention_mask: torch.LongTensor, add_adapter=None
    ):
        ...
        attention_mask = torch.zeros(
            (batch_size, feature_vector_length+past_key_value_length), dtype=attention_mask.dtype, device=attention_mask.device
        )
        # these two operations makes sure that all values before the output lengths idxs are attended to
        attention_mask[(torch.arange(attention_mask.shape[0], device=attention_mask.device), output_lengths+past_key_value_length - 1)] = 1
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = self._get_feature_vector_attention_mask(
                extract_features.shape[1],past_key_values_length, attention_mask[:,past_key_values_length:], add_adapter=False
            )

        hidden_states, extract_features = self.feature_projection(extract_features)

        hidden_states = self._mask_hidden_states(
            hidden_states, mask_time_indices=mask_time_indices, attention_mask=attention_mask[:,past_key_values_length:]
        )
        #增加past_key_value
