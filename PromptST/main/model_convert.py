...
model = SpeechEncoderDecoderModel.from_pretrained("/data/ytf/tmp/model/de_continue/checkpoint-14500")

state_dict=torch.load("/data/ytf/tmp/model/de_continue/checkpoint-14500/pytorch_model.bin",map_location=torch.device('cpu'))
if list(state_dict.keys())[0].startswith('module.'):
    state_dict1 = {k[7:]: v for k, v in state_dict.items()}
    print("over")
model.load_state_dict(state_dict1)
model.save_pretrained("/data/ytf/tmp/model/de_all/fix_encoder_14500")
