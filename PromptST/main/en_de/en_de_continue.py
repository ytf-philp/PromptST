...
tokenized_datasets=load_from_disk("/data/ytf/tmp/dataset_en_de")
...
def compute_metrics(eval_preds):
    ...
    re = re[re['label'].apply(lambda x: x != [""])]
    re = re[re['label'].apply(lambda x: x != ['TO REMOVE]'])]
    re = re[re['label'].apply(lambda x: x != None)]
    re = re[re['pred'].apply(lambda x: x != "")]
    re = re[re['pred'].apply(lambda x: x != 'TO REMOVE]')]
    re = re[re['pred'].apply(lambda x: x != None)]
   
    re=re.dropna().reset_index(drop=True)
    
    #re.to_csv("/data/ytf/PromptST/result/en_de_fix_decoder.csv")
    result = metric.compute(predictions=re["pred"], references=re["label"])
    result = {"bleu": result["score"]}
    #re.to_csv("/data/ytf/PromptST/result/en_de_mid.csv")
...
training_args = Seq2SeqTrainingArguments(
    do_train=True,
    do_eval=True,
    output_dir="/data/ytf/tmp/model/de_prompt_only",
