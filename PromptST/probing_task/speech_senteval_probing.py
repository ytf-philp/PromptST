...
def get_model(device):
    ...
    state_dict=torch.load("/workspace/yutengfei6/users/yutengfei6/docker-remote/prompt/S2T-tuning_new/mid-best-checkpoints/pytorch_model.bin",map_location=torch.device('cpu'))
...
def save_csv(data,label,task,name,sche_step):
    roo="/workspace/yutengfei6/users/yutengfei6/docker-remote/probe/sentence_embedding/"+str(sche_step)+"/"
...
class MDataSet(Dataset):
    def __init__(self, file_path, wav_path):
        ...
        self.get_dataset()
        #self.set_split('tr')
        #print("zzzz", len(self.fulldata), len(self.sent_represents))
        pass
...
class get_embedding():
    ...
    def process_data(self,i,add):
        ...
        for audio_input in self.databatch[i]["audio"]:
            ...
            features.append(outputs.encoder_hidden_states[add]) ###0 5 6 22
        ...
        for feature in features:
            #print(torch.max(feature,1).squeeze().shape)
            max,index=torch.max(feature,1)
            features_avg.append(max.squeeze())
            del feature
            '''
        features_avg=[]
        for feature in features:
...
def test(feature, ck_name, task):
    pass
#Dataset = MDataSet(root+'/data/probing_small/bigram_shift.txt',wav_path+"bigram_shift_result")
#bs将的大数据集切分，一次一次训练，然后梯度下降
